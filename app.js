...
function render() {
  ...
  renderBars("#statusBars", grouped(rows, "MOTIVO")); renderBars("#reasonBars", grouped(rows, "RAZON", true));
  $("#tableHead").innerHTML = `<tr>${columns.map(column => `<th>${labels[column]}</th>`).join("")}</tr>`;
  $("#tableBody").innerHTML = rows.map(row => `<tr>${columns.map(column => `<td>${column === "FECHA" ? formatDate(row[column]) : escapeHtml(row[column] || "—")}</td>`).join("")}</tr>`).join("");
